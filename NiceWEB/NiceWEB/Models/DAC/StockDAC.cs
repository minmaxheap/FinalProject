...
where LOT_DELETE_FLAG is null   AND S.OPERATION_CODE = M.OPERATION_CODE  and (M.OPERATION_CODE is not null or S.PRODUCT_CODE is not null)
...
right join LOT_MATERIAL_HIS m on s.LOT_ID = m.LOT_ID
where s.LOT_DELETE_FLAG is null   AND S.OPERATION_CODE = M.OPERATION_CODE  and (m.OPERATION_CODE is not null or m.PRODUCT_CODE is not null) ");
