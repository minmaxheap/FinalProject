...
namespace NiceWEB.Models
{
	...
	FROM 
	(SELECT CONVERT(VARCHAR,TRAN_TIME,23) TRAN_TIME, CONVERT(varchar, MIN(START_TIME) ,108)WORK_START_TIME,CONVERT(varchar,MAX(END_TIME),108) WORK_END_TIME, 
	DATEDIFF(MINUTE,MIN(START_TIME),MAX(END_TIME)) WORK_TIME FROM LOT_HIS
