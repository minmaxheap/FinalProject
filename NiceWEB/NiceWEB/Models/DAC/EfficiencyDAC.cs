...
namespace NiceWEB.Models
{
	...
	(SELECT CONVERT(VARCHAR,ORDER_DATE,23) ORDER_DATE,  SUM(ORDER_QTY) ORDER_QTY, ISNULL(SUM(PRODUCT_QTY),0) PRODUCT_QTY, ISNULL(SUM(DEFECT_QTY),0)DEFECT_QTY 
	FROM WORK_ORDER_MST 
...
where LOT_DELETE_FLAG <> 'Y' OR LOT_DELETE_FLAG IS NULL  and STORE_CODE is not  null ");
        ...
        public DataTable GetChartData(string from, string to, string workID, string prdCode)
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                cmd.Connection = new SqlConnection(ConfigurationManager.ConnectionStrings["project"].ConnectionString);
