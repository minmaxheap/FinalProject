...
namespace DAC
{
  ...
  WHERE w.PRODUCT_CODE=pm.PRODUCT_CODE AND w.CUSTOMER_CODE=cd.KEY_1 and w.PRODUCT_CODE = r.PRODUCT_CODE and r.FLOW_SEQ =1 and o.OPERATION_CODE = r.OPERATION_CODE  AND CONVERT(DATE, W.CREATE_TIME) = CONVERT(DATE, GETDATE()) AND ORDER_STATUS = 'OPEN'
";
